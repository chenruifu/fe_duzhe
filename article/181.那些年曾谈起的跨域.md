...
若理解了`document.domain`实现跨域原理，那么`location.hash`也就很号理解了，其原理与`document.domain`很相似一样都是动态插入一个`iframe`，然后把`iframe`的`src`指向服务端地址，而服务端同样都是输出一段`JavaScript`代码，同样都是利用和子窗口之间的通信完成数据传输，同样要针对同源策略做出处理。
...
[http://localhost](http://localhost):6000/a.html
...
[http://localhost](http://localhost):7000/b.html
...
[http://localhost](http://localhost):6000/c.html
...
[http://localhost](http://localhost):6000/proxy.html
